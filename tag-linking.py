...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_dataset', type=str, default='', help='Use one of the already generated train and test sets from the data set folder. '
                                                                       'Used for reproduction of results reported in the paper. Argument options are: aker, giza_terms_only, clean and unbalanced, etru')
    ...
    assert params.pretrained_dataset in ['', 'aker', 'giza_terms_only', 'clean', 'unbalanced', 'cognates', 'etru'], 'Allowed arguments are: aker, giza_terms_only, clean, unbalanced, cognates, etru'
    assert params.lang in ['sl', 'fr', 'nl', 'et'], 'Allowed arguments are: sl, fr, nl'
    ...
    if len(pretrained_dataset) == 0:
        ...
        data_train.to_csv('datasets/train_set_et_ru-dataset-200-no-filter.csv', index=False, encoding='utf8', sep=',')
        data_test = createFeatures(data_test, dd, dd_reversed, train=False, cognates=cognates)
        data_test.to_csv('datasets/test_set_et_ru-dataset-200-no-filter.csv', index=False, encoding='utf8', sep=',')
    ...
    if filter:
        data_train = filterTrainSet(data_train, trainset_balance, 0, cognates=cognates)
