...
def createTrainingAndTestingExamplesBuccEurovoc(df_term_pairs, term_freq_tar, term_freq_src, neg_train_count, neg_test_count, giza_check = False):
    term_pairs = list(zip(df_term_pairs['SRC'].tolist(), df_term_pairs['TAR'].tolist()))
    if giza_check:
        term_pairs_giza = []
        for src_term, tar_term in term_pairs:
...
def createTrainingAndTestingExamplesBucc(df_term_pairs, en_terms, fr_terms, neg_train_count, neg_test_count, giza_check = False):
    term_pairs = list(zip(df_term_pairs['SRC'].tolist(), df_term_pairs['TAR'].tolist()))
    ...
    for en_term in en_terms:
        for fr_term in fr_terms:
            candidates.append(en_term+"\t"+fr_term)
    ...
    counter = neg_train_count * len(train_set)
    while counter > 0:
        neg_example = random.choice(candidates)
        if neg_example not in existing_pairs:
            train_set.append((neg_example.split("\t")[0], neg_example.split("\t")[1], 0))
            existing_pairs.add(neg_example)
            candidates.remove(neg_example)
            counter -= 1
    df_train = pd.DataFrame(train_set, columns=['src_term', 'tar_term', 'label'])
    ...
    counter = neg_test_count * len(test_set)
    distinct_neg_terms = set()
    while counter > 0:
        neg_example = random.choice(candidates)
        if neg_example not in existing_pairs and neg_example not in distinct_neg_terms:
            test_set.append((neg_example.split("\t")[0], neg_example.split("\t")[1], 0))
            distinct_neg_terms.add(neg_example)
            candidates.remove(neg_example)
            counter -= 1
    df_test = pd.DataFrame(test_set, columns=['src_term', 'tar_term', 'label'])
    df_pos = df_test[df_test['label'] == 1]
...
def filterTrainSet(df, ratio, fasttext_topn, cognates=False):
    ...
    if fasttext_topn == 0:
    ...
    elif fasttext_topn > 0:
        print('I am at fasttext')
        #df_pos_dict = df_pos[df_pos['isFirstWordTranslated'] == 1]
        #df_pos_dict = df_pos_dict[df_pos['isLastWordTranslated'] == 1]
        #df_pos_dict = df_pos_dict[df_pos['isFirstWordTranslated_reversed'] == 1]
        #df_pos_dict = df_pos_dict[df_pos['isLastWordTranslated_reversed'] == 1]
        ...
        df_pos = pd.concat([df_pos_fasttext_1, df_pos_fasttext_n])
...
def build_manual_eval_set2(terms_src, terms_tar, iter_src, iter_tar):
...
if __name__ == '__main__':
    ...
    if filter:
        fasttext_topn = 0
        data_train = filterTrainSet(data_train, trainset_balance, fasttext_topn, cognates=cognates)
        data_train.to_csv('new-filter-balance-10-train.csv', encoding="utf8", index=False, sep='\t')
